          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-500">
              <span className="flex items-center gap-1">
                ...
                {mcpData.codeSnippets.length} {t('mcpManager.codeSnippets')}
              </span>
              <span className="flex items-center gap-1">
                <Tag className="h-4 w-4" />
                {mcpData.metadata.tags.length} {t('mcpManager.tags')}
              </span>
              <span className="flex items-center gap-1">
      ...
      {isExpanded && (
        <CardContent>
          <div className="space-y-4">
            {mcpData.metadata.description && (
              <div>
                <h4 className="font-medium text-gray-700 mb-2">{t('mcpManager.description')}</h4>
                <p className="text-gray-600">{mcpData.metadata.description}</p>
              </div>
            )}
            
            <div>
              <h4 className="font-medium text-gray-700 mb-2">{t('mcpManager.labels')}</h4>
            ...
            <div>
              <h4 className="font-medium text-gray-700 mb-2">{t('mcpManager.snippetDescription')}</h4>
              <div className="grid gap-2 max-h-32 overflow-y-auto">
                ...
                {mcpData.codeSnippets.length > 3 && (
                  <div className="text-center text-gray-500 text-sm">
                    {t('mcpManager.moreCodeSnippets', { count: mcpData.codeSnippets.length - 3 })}
                  </div>
                )}
              </div>
            </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-1 gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  type="text"
                  placeholder={t('common.search') + t('mcpManager.title') + '...'}
                  value={searchTerm}
              ...
              >
                <option value="">{t('common.all')} {t('common.tools')}</option>
          ...
          {availableTags.length > 0 && (
            <div className="mt-4">
              <div className="text-sm font-medium text-gray-700 mb-2">{t('mcpManager.labels')}:</div>
      ...
      <div className="grid gap-4">
        {filteredData.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">{t('mcpManager.noData')}</h3>
            <p className="text-gray-600">
              {mcpData.length === 0 ? t('mcpManager.importData') : t('mcpManager.noMatch')}
            </p>
          </div>
        ) : (
